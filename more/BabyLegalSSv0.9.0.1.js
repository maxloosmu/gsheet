// https://stackoverflow.com/questions/45422906/why-do-both-onchange-and-onedit-triggers-fire-at-the-same-time
function onChange(e) {
  let c = e.range;
  let sheet = SpreadsheetApp.getActiveSheet();
  let h = new History();
  selectedRange = sheet.getRange(3, 2, 100, 8);
  // This function below leads to endless flip flop
  // of checkbox value:
  if (e.changeType != "EDIT") {
    // forEachCellInRange(selectedRange, h, sheet);
  }
}
function onEdit(e) {
  ...
  let sheet = SpreadsheetApp.getActiveSheet();
  let h = new History();
  // SpreadsheetApp.getUi().alert(Object.getOwnPropertyNames(e));
...
function startProcessing(c, h, sheet) {
  // SpreadsheetApp.getUi().alert("in startProcessing");
...
function forEachCellInRange(range, h, sheet) {
  const numRows = range.getNumRows();
  const numCols = range.getNumColumns();
  for (let i = 1; i <= numCols; i++) {
    for (let j = 1; j <= numRows; j++) {
      let cell = range.getCell(j, i);
      if (isKeyword(cell.getValue())) {
        SpreadsheetApp.getUi().alert("keyword detected");
        // cell.setValue("Changed");
        let newC = startProcessing(cell, h, sheet);
        j = newC.getRowIndex()-3;
        i = 1;
      }
    }
  }
...
function findStart(c) {
  ...
  // SpreadsheetApp.getUi().alert(c.getValue());
  let nextCol = 0;
  let maxCount = 5;
  while (nextCol < maxCount) {
    ...
    if (isKeyword(gtr)) {
      // SpreadsheetApp.getUi().alert(gtr);
      c = getTopRight;
      return findStart(c);
    }
    nextCol++;
  ...
  if (nextCol == maxCount) {
    ...
    if (isKeyword(gtl)) {
      // SpreadsheetApp.getUi().alert(gtl);
      c = getTopLeft;
      return findStart(c);
    }
  }
