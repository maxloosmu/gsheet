...
function petriLink() {
  ...
  let url = "http://18.139.62.80:8080/petri/" + cachedUuid;
...
function getPetri() {
  ...
  // // Problem with file+folder generation is too many clones.
  // // This generates file+folder in Spreadsheet's Google Drive:
  // let thisFileId = SpreadsheetApp.getActiveSpreadsheet().getId();
  // let thisFile = DriveApp.getFileById(thisFileId);
  // let parentFolder = thisFile.getParents();
  // let title = "";
  // let folder;
  // while (parentFolder.hasNext()) {
  //   folder = parentFolder.next();
  //   title = folder.getName();
  // }
  // // This generates file+folder in local Google Drive:
  // rootFolder = DriveApp.getRootFolder();
  // let petriFolder = rootFolder.createFolder('PetriFolder');   // 1° way to create sub folder
  // let petriFile = petriFolder.createFile("PetriFile", response);
  // ui.prompt(petriFile.getName() + " created in " + petriFolder.getName());
  // // Logger.log(petriFile.getName());
  return response;
}
function getCommand() {
  let userCache = CacheService.getUserCache();
