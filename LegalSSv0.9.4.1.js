...
function corel4Link() {
  ...
  let url = "http://18.139.62.80:8080/corel4/" + cachedUuid;
  let urlString = HtmlService.createHtmlOutput("<a href='" + url + "' target='_blank'>Core L4 Representation</a>").getContent();
  return urlString;
}
function petriLink() {
  saveUuid();
  ...
  let url = "http://18.139.62.80:8080/petri/" + cachedUuid; // + "/" + spreadsheetId + "/" + sheetId;
  let urlString = HtmlService.createHtmlOutput("<a href='" + url + "' target='_blank'>Petri Net</a>").getContent();
  // ui.prompt(urlString);
  return urlString;
}
function aasvgLink() {
  ...
  let userCache = CacheService.getUserCache();
  let cachedUuid = userCache.get("uuid");
  let url = "http://18.139.62.80:8080/aasvg/" + cachedUuid;
...
function jsonLink() {
  ...
  let userCache = CacheService.getUserCache();
  let cachedUuid = userCache.get("uuid");
  let url = "http://18.139.62.80:8080/json/" + cachedUuid;
  let urlString = HtmlService.createHtmlOutput("<a href='" + url + "' target='_blank'>CSV convert to JSON</a>").getContent();
  return urlString;
}
function getCommand() {
  let userCache = CacheService.getUserCache();
...
function exportCSV() {
  ...
  let cachedUuid = userCache.get("uuid");
  saveCommands();
  let cachedCommand = getCommand();
  let spreadsheetId = "";
  let sheetId = 0;
  [spreadsheetId, sheetId] = getSsid();
  let sheet = SpreadsheetApp.getActiveSheet();
  ...
  let response = UrlFetchApp.fetch('http://18.139.62.80:8080/post', options);
...
function onEdit(e) {
  ...
  // added by wmw 20220701 because sometimes the onEdit changes get in the way
